        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-white/10 px-4 py-2 rounded-full mb-6 border border-white/20">
              <LeafIcon className="w-5 h-5 text-[#F9A825]" />
              <span className="text-[#F9A825] font-semibold">Australia&apos;s Clean Energy State</span>
            </div>

            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-6">
              <span className="text-white">Solar Battery Installation</span>
              <br />
              <span className="text-[#F9A825]">Tasmania</span>
            </h1>

            <p className="text-xl text-white/90 max-w-3xl mx-auto mb-8">
              Tasmania&apos;s excellent solar potential and remote areas make battery storage ideal for energy independence. Join 600+ Tasmanian households already saving with Super Solar Energy.
            </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-3xl sm:text-4xl font-bold text-[#333333] mb-4">
              Why Tasmania Homes Need <span className="text-[#E8621A]">Battery Storage</span>
            </h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: LeafIcon, title: "Clean Energy State", description: "Tasmania has excellent solar potential year-round. Maximize renewable energy self-sufficiency", color: "text-[#E8621A]" },
              { icon: CurrencyIcon, title: "TAS Energy Costs", description: "Tasmanian electricity prices are 24-29c/kWh. Solar batteries reduce your energy bills significantly", color: "text-[#E8621A]" },
              { icon: ShieldIcon, title: "Remote Area Security", description: "Tasmania's rural areas benefit from battery backup during weather events and outages", color: "text-[#E8621A]" },
              { icon: BoltIcon, title: "Energy Independence", description: "Reduce grid dependence and achieve energy independence for remote properties", color: "text-[#E8621A]" }
            ].map((benefit, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
      ...
      <section className="relative py-20 bg-white">
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl font-bold text-[#333333] mb-4">Tasmania Battery <span className="text-[#E8621A]">Packages</span></h2>
            <p className="text-[#666666] max-w-2xl mx-auto">Competitive pricing for Tasmanian residents with full installation included</p>
          </motion.div>

          <div className="grid md:grid-cols-3 gap-8">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
              ...
              <Link href="/quote" className="bg-[#E8621A] text-white hover:bg-[#c74f12] w-full text-center block py-3 rounded-lg font-semibold transition-colors">Get Tasmania Quote</Link>
            </motion.div>

            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="relative bg-white rounded-xl shadow-xl border-2 border-[#E8621A] p-8 transform md:scale-105">
              ...
              <Link href="/quote" className="bg-[#F9A825] text-[#0f1923] hover:bg-[#e09000] w-full text-center block py-3 rounded-lg font-semibold transition-colors">Get Tasmania Quote</Link>
            </motion.div>

            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
              <div className="text-center mb-6">
                <BatteryIcon className="w-12 h-12 text-[#E8621A] mx-auto mb-4" />
          ...
          <p className="text-center text-[#666666] mt-8 text-sm">*Prices include installation. Final price depends on site assessment and system selection.</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            <div className="inline-flex items-center gap-2 mb-4">
              ...
              <h2 className="text-3xl sm:text-4xl font-bold text-[#333333]">Tasmania <span className="text-[#E8621A]">Service Areas</span></h2>
            </div>
            <p className="text-[#666666] max-w-2xl mx-auto">We install solar batteries across Tasmania - from Hobart to Launceston and beyond</p>
          </motion.div>

          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {tasmaniaAreas.map((area, index) => (
                <div key={index} className="text-center py-2 px-3 rounded-lg bg-gray-100 hover:bg-gray-200 transition-colors">
                  <span className="text-[#444444] text-sm">{area}</span>
                </div>
              ))}
            </div>
            <p className="text-center text-[#666666] mt-6 text-sm">
              Don&apos;t see your area? We service all Tasmania regions. <Link href="/contact" className="text-[#E8621A] hover:underline">Contact us</Link> to confirm.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-[#0f1923] mb-4">
              Ready for Tasmania&apos;s <span className="text-[#E8621A]">Clean Energy Future</span>?
            </h2>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/quote" className="bg-[#0f1923] text-white hover:bg-[#1a2a3a] inline-flex items-center gap-2 text-lg px-8 py-4 rounded-lg font-semibold transition-colors">
                Get Free Tasmania Quote
                <ArrowRightIcon className="w-5 h-5" />
              </Link>
              <a href="tel:1300090984" className="bg-white text-[#0f1923] hover:bg-gray-100 inline-flex items-center gap-2 text-lg px-8 py-4 rounded-lg font-semibold transition-colors">
                <PhoneIcon className="w-5 h-5" />
                Call 1300 09 09 84
              </a>
            </div>
